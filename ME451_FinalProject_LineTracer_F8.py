...
def control_input(Alpha, Q, R, X, t):
	...
	# Add input noise
	# U += np.random.uniform(-25.0, 25.0, size=(2, 1))
...
# 3) Initial condition
Xd = np.array([[0, 0, np.pi/4]]).T 			# Desired state vector. Currently contains X0_d only
X = np.array([[0, 0, np.pi/4]]).T 		# Actual state vector. Currently contains X0 only
